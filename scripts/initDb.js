...
const createTables = async () => {
  try {
    ...
    console.log("Eliminando tablas existentes para reiniciar la base de datos...");
    await client.query(`
    ...
    console.log("Creando tablas con soporte Multi-Tenant (Organizaciones)...");
    ...
    // Create instances table with organization_id
    await client.query(`
    ...
    `);
    console.log("Tabla 'instances' creada.");

    // Create notification_logs table with organization_id
    await client.query(`
    ...
    // Create team_members table with organization_id
    await client.query(`
    ...
    `);
    console.log("Tabla 'team_members' creada.");

    // Create a default organization for the seed data
    console.log("Creando Organización de demostración...");
    const orgRes = await client.query(`
      INSERT INTO organizations (name) VALUES ('Konsul Demo SA') RETURNING id
    `);
    ...
    for (const member of seedTeam) {
      await client.query(
        `INSERT INTO team_members (id, organization_id, name, role, email, avatar, assigned_processes, department, manager_id)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
        [member.id, orgId, member.name, member.role, member.email, member.avatar, JSON.stringify(member.assigned_processes), member.department, member.manager_id]
      );
    }
    ...
    for (const temp of seedTemplates) {
      await client.query(
        ...
        [temp.id, orgId, temp.title, temp.description, temp.duration_days, temp.companion_name, temp.companion_avatar, temp.companion_greeting, temp.category, JSON.stringify(temp.steps)]
      );
    }
